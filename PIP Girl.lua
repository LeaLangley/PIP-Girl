...
menu.toggle_loop(Session, "Clear Traffic", {"antitraffic"}, "Clears the traffic on the session for everyone.", function()
    if fix_multiplier_id then
        if MISC.DOES_POP_MULTIPLIER_SPHERE_EXIST(fix_multiplier_id) then
            MISC.REMOVE_POP_MULTIPLIER_SPHERE(fix_multiplier_id, false)
        end
    end
    ...
    if transitionState(true) == 1 then
        ...
        if not pop_multiplier_id then
            pop_multiplier_id = MISC.ADD_POP_MULTIPLIER_SPHERE(0.0, 0.0, 0.0, 19666, 0.0, 0.0, false, true)
    ...
    fix_multiplier_id = MISC.ADD_POP_MULTIPLIER_SPHERE(0.0, 0.0, 0.0, 19666, 1.0, 1.0, false, true)
end)
